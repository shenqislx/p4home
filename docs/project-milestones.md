...
跨角色产品召回。代表性家庭数据、HA clean-commit 复跑、P4 Cat/Voice 端到端和长期 SQLite
...
HA 真值且无 service call。HA 结果仍为 `local_precommit`，需 clean commit 复跑；真实
家庭数据、P4/Voice、SQLite 生产耐久和身份模型仍为 `pending`。

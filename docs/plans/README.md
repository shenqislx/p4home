...
丢帧 0、HA/固定命令与稳态 UI 主链不回归，并在最终独立 review 后关闭技术门禁。Mac 系统扬声器只
替代口播输入，P4 startup tone 的独立可听人工观察仍明确待补，不能由 `tone_played` marker 冒充。
当前进入 5C STT/VAD 与统一 Role Runtime；默认 SR 仍关闭，尚未接入真实 TTS。

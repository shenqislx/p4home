...
async function main(): Promise<void> {
  if (requiredEnvironment("P4HOME_HARDWARE_PROFILE") !== "phase5e_e2e") {
  ...
  await using store = new SqliteAuditStore(requiredEnvironment("P4HOME_PHASE5E_AUDIT_DB"));
  ...
  try {
    ...
    const provider = {
      async chat(request: Parameters<typeof deterministic.chat>[0]) {
        if (request.tools !== undefined
            && classifyPhase5ePrompt(request.messages.at(-1)?.content ?? "", prompts) === "write") {
          writeMayHaveOccurred = true;
        }
        return await deterministic.chat(request);
      },
    };
    ...
    const pythonTts = new PythonTtsProvider({
      python_executable: requiredEnvironment("P4HOME_TTS_PYTHON"),
      worker_script: requiredEnvironment("P4HOME_TTS_WORKER"),
      model_path: requiredEnvironment("P4HOME_TTS_MODEL"),
    ...
    const measuredTts: TtsProvider = {
      async synthesize(request, options): Promise<TtsSynthesisResult> {
        const started = performance.now();
        try { return await pythonTts.synthesize(request, options); }
        finally { ttsDurations.push(performance.now() - started); }
      },
    };
    ...
    const dispatcher = new UnifiedVoiceRoleDispatcher({
      ...
      sessions: new RoleSessionRegistry({
        robot: "phase5e-session-robot",
        human: "phase5e-session-human",
        cat: "phase5e-session-cat",
      }),
      scheduler,
      timeout_ms: 30_000,
      audit: { store },
    ...
    const ttsPipeline = new RoleAwareTtsPipeline(measuredTts);
    ...
    if (voiceResults.length !== 4 || roleResults.size !== 4) throw new Error("interaction_count");
    const interactions: Phase5eGateInteraction[] = voiceResults.map((voice) => {
      const record = roleResults.get(voice.interaction_id);
      if (record === undefined) throw new Error("role_result_missing");
      const kind = classifyPhase5ePrompt(record.interaction.text, prompts);
      if (kind === null) throw new Error("holdout_transcript_mismatch");
    ...
    await atomicJson(resultFile, {
      ...
      interactions: interactions.map((item) => ({
        kind: item.kind,
        role_id: item.role.run.role_id,
        role_status: item.role.run.status,
      ...
      tts_total_ms: Math.round(ttsDurations.reduce((sum, value) => sum + value, 0)),
      audit_events: auditEvents,
      restored: true,
      ...verdict,
    });
    process.stdout.write(
      `VERIFY:phase5e:voice_e2e:PASS interactions=4 stt_calls=${sttDurations.length} `
      + `tts_calls=${ttsDurations.length} playback_segments=${verdict.playback_segments} `
      + `playback_bytes=${verdict.playback_bytes} audit=persisted restored=yes `
...
void main().catch((error: unknown) => {
  ...
  process.stdout.write(`VERIFY:phase5e:voice_e2e:FAIL reason=${reason}\n`);
  process.exitCode = 1;
});

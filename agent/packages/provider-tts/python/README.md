...
`zf_xiaoxiao` (Human) and `zf_xiaobei` (Robot), with `zm_yunxi` retained as a
snapshot asset (not an automatic runtime fallback). It accepts bounded, serialized JSON
...
weights, the approved voice files, and their SHA-256 manifest. The worker
